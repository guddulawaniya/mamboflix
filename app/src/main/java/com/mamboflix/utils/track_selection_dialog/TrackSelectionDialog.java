...
import com.mamboflix.ui.activity.TestHLSActivity;
...
public final class TrackSelectionDialog extends DialogFragment implements TestHLSActivity.updateTrack {
  private static String TAG = "TrackSelectionDialog";
  private final SparseArray<TrackSelectionViewFragment> tabFragments;
  private final ArrayList<Integer> tabTrackTypes;
  private static String selectedTrack="Auto";
  private int titleId;
        ...
        /* onClickListener= */ (dialog, which) -> {
          ...
          for (int rendererIndex = 0; rendererIndex < mappedTrackInfo.getRendererCount(); rendererIndex++) {
            ...
            if (!overrides.isEmpty()) {
              ...
              Format tg=mappedTrackInfo.getTrackGroups(rendererIndex).get(0).getFormat((overrides.get(0).tracks[overrides.get(0).tracks.length-1]));
//              Toast.makeText(trackSelectionDialog.requireActivity(), tg.toString(), Toast.LENGTH_SHORT).show();
                // Change by lavkush mishra
              /*selectedTrack= String.valueOf(tg.width+" x "+tg.height);*/
                selectedTrack= String.valueOf(tg.width+"p");
            }

            else if(overrides.isEmpty() && count[0] == 0) {
              selectedTrack="Auto";
            }
